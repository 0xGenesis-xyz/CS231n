...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    H1, cacheH1 = affine_relu_forward(X, W1, b1)
    scores, cacheOut = affine_forward(H1, W2, b2)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dH1, dW2, db2 = affine_backward(dout, cacheOut)
    _, dW1, db1 = affine_relu_backward(dH1, cacheH1)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(1, self.num_layers):
      ...
      if self.use_dropout:
        H, dpCaches[i] = dropout_forward(H, self.dropout_param)
    ...
    ############################################################################
    loss, dout = softmax_loss(scores, y)
    dH, dW, db = affine_backward(dout, caches[self.num_layers])
    ...
    for i in xrange(self.num_layers-1, 0, -1):
      if self.use_dropout:
        dH = dropout_backward(dH, dpCaches[i])
      if self.use_batchnorm:
